...
uint8_t BYTE_ARRAY_0067a0a8[256] = { 0 };

BYTE g_keyState[256];
BYTE g_inputBuffer1[256];
...
int updateKeyboardState(BYTE* keyboardStateOut)
{
    ...
    for (int i = 0; i < 256; ++i)
    {
        BYTE previousState = g_inputBuffer1[i];
        BYTE currentState = g_keyState[i] & 0x80;

        g_keyState[i] = currentState;
        g_previousKeyState[i] = previousState;

        BYTE newState = currentState & ~previousState;
        g_inputBuffer1[i] = currentState;
        g_keyState[i] = newState;
